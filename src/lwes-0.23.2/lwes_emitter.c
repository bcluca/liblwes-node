   ...
   LWES_INT_32 _js_emitter)
{
  ...
  /* Save reference to Javascript Emitter wrapper */
  emitter->_js_emitter = _js_emitter;
...
lwes_emitter_emit_bytes
  (struct lwes_emitter *emitter,
   ...
   size_t length)
{
  // Bypassing original implementation as networking support is missing
  // return lwes_net_send_bytes (&(emitter->connection), bytes ,length);

  return js_send_bytes(emitter->_js_emitter, bytes, length);
}
